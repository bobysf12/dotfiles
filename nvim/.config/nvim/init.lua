...
-- When pasting in visual mode, do not overwrite the default register with the replaced text
vim.keymap.set("x", "p", '"_dP')

-- Keymap for splitting the window vertically
vim.api.nvim_set_keymap("n", "<leader>wv", ":vsplit<CR>", { noremap = true, silent = true })

-- Keymap for splitting the window horizontally
vim.api.nvim_set_keymap("n", "<leader>wh", ":split<CR>", { noremap = true, silent = true })

vim.keymap.set("n", "yc", "yygccp", { remap = true, desc = "Comment line and make copy on the next line" })
...
require("lazy").setup({
	...
	{ "Bilal2453/luvit-meta", lazy = true },
	{
		...
		dependencies = {
			...
			-- Blink CMP with optional setup (customize via `opts` if needed)
			{ "saghen/blink.cmp", opts = {} },

			-- Required for default LSP capabilities
			"hrsh7th/cmp-nvim-lsp",

			-- Optional but useful UI improvements
			{ "j-hui/fidget.nvim", opts = {} },
		},

		config = function()
			-- Setup event for LSP attachment to buffers
			...
			-- Create base capabilities from cmp-nvim-lsp
